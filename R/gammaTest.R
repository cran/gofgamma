...
test.KS <- function(data,boot=500,alpha=0.05){
  if (!is.vector(data)){warning("data must be of type vector")}
  para=gamma_est(data)
  cv=crit.values(samplesize=length(data),statistic=KS,tuning=NULL,k_estimator=para[1],boot.param=boot,alpha=alpha)
  Test.value=KS(data/para[2],para[1])
  result <- list("Test" = "KS","parameter" = NULL, "T.value"=Test.value,"cv"=cv,"par.est"=para,"Decision"=Test.value>cv,"sig.level"=alpha,"boot.run"=boot)
  attr(result, "class") <- "gofgamma"
  return(result)
}
...
test.CM <- function(data,boot=500,alpha=0.05){
  para=gamma_est(data)
  cv=crit.values(samplesize=length(data),statistic=CM,tuning=NULL,k_estimator=para[1],boot.param=boot,alpha=alpha)
  Test.value=CM(data/para[2],para[1])
  result <- list("Test" = "CM","parameter" = NULL, "T.value"=Test.value,"cv"=cv,"par.est"=para,"Decision"=Test.value>cv,"sig.level"=alpha,"boot.run"=boot)
  attr(result, "class") <- "gofgamma"
  return(result)
}
